...
void generateTestData(const string& filename) {
    ...
    random_device rd;
    mt19937 gen(rd());
    normal_distribution<double> scoreDist(435.0, 100.0); // Mean 435, stddev 100
    ...
    vector<pair<string, int>> students; // id, score (integer)
    vector<string> collegeIds;
    vector<pair<string, int>> colleges; // id, quota
    vector<vector<string>> studentPrefs(numStudents);

    // Generate students with normally distributed integer scores
    for (int i = 0; i < numStudents; ++i) {
        int score;
        do {
            score = static_cast<int>(round(scoreDist(gen)));
        } while (score < 120 || score > 750); // Truncate to [120, 750]
        students.emplace_back("S" + to_string(i), score);
    }
